...
export default function CarBookings() {
  ...
  // fetch bookings when tab, page, or search change
  useEffect(() => {
  ...
  // fetch available cars when Add dialog opens or dates change
  useEffect(() => {
    if (!openAdd) return;
    const params = {};
    ...
    },
    {
      ...
      render: (_,r) => tab === 0
        ? <AiOutlineEdit
            ...
            onClick={() => {
              setEditB({ id: r.id, status: r.status });
              setOpenEdit(true);
            }}
          />
        : r.status
      ...
      <Dialog open={openAdd} onClose={() => setOpenAdd(false)} fullWidth maxWidth="sm">
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={handleAdd}
            disabled={cars.length === 0} // disable if no cars
          >
            Create
          </Button>
        </DialogActions>
